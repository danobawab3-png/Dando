...
@app.route('/')
def home():
    return "تم إصلاح الثغرة البرمجية نهائياً والبوت يعمل 24 ساعة!"

def run_web_server():
    app.run(host='0.0.0.0', port=8080)

# ⚠️ بيانات التلغرام الصحيحة والمضمونة الخاصة بك
...
def main_scanner_loop():
    print("🤖 بدأ محرك الكسح المصلح العمل بصفة رسمية...")
    seen_tokens = set()
    
    # 🔥 اختبار فوري ومباشر: سيبعث الرسالة فوراً في أول ثانية لفك أي عقدة اتصال!
    send_to_telegram("🚀 *تم تفعيل خط القنص السحابي بنجاح!* البوت مستيقظ الآن ويراقب مجمعات سيولة Solana على مدار الساعة...")
    
    while True:
        try:
            res = requests.get(SOLANA_BOOSTS_API)
            if res.status_code == 200:
                # معالجة تركيبة البيانات الجديدة (سواء كانت قائمة أو قاموس يحتوي على data)
                data = res.json()
                tokens_list = data if isinstance(data, list) else data.get('data', [])
                ...
                if isinstance(tokens_list, list):
                    for token_data in tokens_list:
                        ...
                        if not pairs_list or not isinstance(pairs_list, list): continue
                        
                        # استخراج المسبح الأول بشكل صحيح
                        pair = pairs_list[0]
                        ...
                        # 📢 بث الإشارة الحية الفورية إلى تلغرام بالتنسيق الصحيح
