...
@app.route('/')
def home():
    return "البوت شغال 24 ساعة بأمان وسرية تامة!"
...
def send_to_telegram(text):
    ...
    try:
        ...
        # طباعة النتيجة في لوحة تحكم Render للتأكد
        print(f"استجابة تلغرام: {res.status_code} - {res.text}")
    except Exception as e:
        print(f"خطأ في إرسال تلغرام: {e}")

def main_scanner_loop():
    print("🤖 بدأ البوت بمراقبة شبكة Solana بتعديلات الزخم الذكية والتنسيق الصحيح...")
    seen_tokens = set()
    
    # إرسال رسالة ترحيبية فورية للتأكد من أن الاتصال شغال تماماً الآن
    send_to_telegram("🚀 *تم إطلاق البوت بنجاح!* فلاتر التداول الذكية مستيقظة وتراقب الشبكة الآن...")
        ...
        try:
            ...
            if res.status_code == 200:
                for profile in res.json():
                    ...
                    # 📢 إرسال التنبيه الفوري بالتنسيق الجديد المبسط والمضمون
            ...
            print(f"حدث خطأ في حلقة الفحص: {e}")
        time.sleep(15)
